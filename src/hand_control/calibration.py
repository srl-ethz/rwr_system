...
class CalibrationClass():
    ...
    def move_to_limit_and_get_pos(self, motor_start, position_increment=0.1, threshold=0.0005):
        """
        ...
        self.write_desired_motor_current(170 * np.ones(len(self.motor_ids)))
    ...
    def auto_calibrate_fingers_with_pos(self, calib_current=40, maxCurrent: int = 150):
            """
            Calibrate each finger by extending the MCP joint fully in both directions and recording the motor positions.
            """
            calib_current = 150 
            ...
            self.motor_id2init_pos = self.move_to_limit_and_get_pos(-1*motor_pos_calib*motors_directions)
            ...
            for muscle_group in self.muscle_groups:
                ...
                if muscle_group.name in ["index", "middle", "ring", "pinky", "thumb"]:
                    ...
                    mcp_pos_flexed = self.move_to_limit_and_get_pos(-1*motor_pos*motors_directions)[mcp_motors_id_map_idx]
                    ...
                    motor_pos_res = self.move_to_limit_and_get_pos(-1*motor_pos*motors_directions)
